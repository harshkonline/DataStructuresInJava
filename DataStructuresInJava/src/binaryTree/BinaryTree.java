        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 'v':
                	...
                	System.out.println(isValid(theTree.root)+"@@@");

                    break;
                case 'l':

                	System.out.println(LCA(theTree.root, theTree.root.leftNode.leftNode, theTree.root.rightNode.rightNode).data+" LCA!!");
...
//-------------------------------------------------------------
   ...
   private static boolean isValidBST(node node, int MIN, int MAX) {
        ...
        if(node.data > MIN 
            && node.data < MAX
            && isValidBST(node.leftNode, MIN, Math.min(node.data,MAX))
            && isValidBST(node.rightNode, Math.max(node.data,MIN), MAX))
            return true;
        else 
            return false;
   }
   ...
   //starting from root, going downwords, if any node has either p or q as direct child then its LCA
   public static node LCA(node root, node a, node b){
	   ...
	   left=LCA(root.leftNode,a,b);
	   right=LCA(root.rightNode,a,b);
